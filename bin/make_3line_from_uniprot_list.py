...
parser.add_argument("uni_list", type=argparse.FileType('r'), help="List of Uniprot IDs")
...
for line in args.uni_list:
    pid = line.strip()
    if pid in uniprot_to_pdb_map:
        capture_pdb = ''

        # Reverse so potential newer structures are used
        for pdb_id in reversed(uniprot_to_pdb_map[pid]):
            # Prefer the 'A' chain 
            if pdb_id[-1] == 'A':
                capture_pdb = pdb_id
                break
        if not capture_pdb:
            # If we haven't captured an 'A' chain but there are chains,
            # take the last one (potentially newer)
            capture_pdb = uniprot_to_pdb_map[pid][-1]

        mapped_ids.append([pid, capture_pdb])
out_text = ""

for uniprot_id, pdb_id in mapped_ids:
    header = '>' + uniprot_id + '|' + pdb_id
