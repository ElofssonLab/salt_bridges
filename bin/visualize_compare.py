...
parser.add_argument("charges_mem", type=str, help="In charge pickle mem")
parser.add_argument("charges_glob", type=str, help="In charge pickle glob")
...
in_pickle_mem = args.charges_mem
in_pickle_glob = args.charges_glob
# out_image = in_pickle.split('/')[-1].split('.')[0] + "_AA_vis.svg"
out_image = args.out
mem_aaCount, mem_aaPairs, mem_aaHits = pickle.load(open(in_pickle_mem, 'rb'))
glob_aaCount, glob_aaPairs, glob_aaHits = pickle.load(open(in_pickle_glob, 'rb'))
    # open('data/chargeDataGlobularFromPDBRed50TrimmedLen15v2.pickle',
    #      'rb'))
log_odds_text_list = ["Separation\tFirst\tSecond\tlogOdds\terror\tp"]
mem_logOdds = np.zeros([10, 20, 20])
glob_logOdds = np.zeros([10, 20, 20])
ci = []
...
mem_mask = np.zeros([10, 20, 20])
glob_mask = np.zeros([10, 20, 20])
for i in range(8):
    for first in range(20):
        for second in range(20):
            ...
            d = mem_aaPairs[i]**2  # - c  # same as above, not odds
            # print(a, b, c, d)
            odds = (a / b) / (c / d)
            if odds == 0.0:
            ...
            mem_logOdds[i][aas.index(aasOri[first])][aas.index(aasOri[second])]\
                = logOddsValue
            ...
            # if i == 0:
            mem_mask[i][aas.index(aasOri[first])][aas.index(aasOri[second])]= 1 if psf > 0.05 else 0
...
for i in range(8):
    for first in range(20):
        for second in range(20):
            ...
            # d is aaPairs for this step times 2 for each of the two
            # amino acids as each pair contains two AAs.
            d = glob_aaPairs[i]**2  # - c  # same as above, not odds
            # print(a, b, c, d)
            odds = (a / b) / (c / d)
            if odds == 0.0:
                logOddsValue = -math.inf
                ...
                logOddsValue = math.log(odds)
            glob_logOdds[i][aas.index(aasOri[first])][aas.index(aasOri[second])]\
                = logOddsValue
            ...
            # if i == 0:
            glob_mask[i][aas.index(aasOri[first])][aas.index(aasOri[second])]= 1 if psf > 0.05 else 0
            # print("{}-{} {}: logOdds: {:.3f} SE: {:.3f} p: {:.2e}".format(aasOri[first], aasOri[second],i+1, logOddsValue, SE, psf))
            log_odds_text_list.append("{}\t{}\t{}\t{:.3f}\t{:.3f}\t{:.2e}".format(i+1, aasOri[first], aasOri[second],logOddsValue, SE, psf))
            ci.append((math.log(odds)+1.96*math.sqrt(1/a+1/b+1/c+1/d), math.log(odds)-1.96*math.sqrt(1/a+1/b+1/c+1/d), psf))
            # if psf < 0.05:
...
logOdds = mem_logOdds - glob_logOdds
# print(mem_mask.shape, glob_mask.shape)
mask = np.logical_or(mem_mask, glob_mask)
for o in [False, True]:
